...
%     if par.probe_modes > par.Nscans  && par.Nscans <= 1 % don't run for multiscan 
%        reconstruct_ind = [self.reconstruct_ind{:}];
%         if par.variable_probe
%             plotting.smart_figure(i+1)
%             clf       
%             power  = power / sum(power);
%              for i = 1:length(self.probe)
%                 pos = self.modes{i}.probe_positions; % fixed a bug from .probe to . modes by ZC
%                 pos = pos(:,[2,1]); 
%                 pos(:,1) = -pos(:,1);
%                 subplot(2,1,1)
%                 hold all
