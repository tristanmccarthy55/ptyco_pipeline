...
Sec.~\ref{sec:pol}. The localisation code and its protocol are self-contained within the
project repository,\footnote{\url{https://github.com/tristanmccarthy55/ptyco_pipeline}; the localisation
package and its protocol are under \texttt{analysis/atomfind/}, with step-by-step instructions
in \texttt{PEER.md}. The input archive is distributed alongside it, being too large to version
with the code.} together with a checksummed archive of four inputs: the reconstructed phase volume as a
$70\times404\times404$ complex array, the measured lead and titanium kernels, and the reference
structure used for scoring, supplied both as deposited and pre-transformed into the beam frame.
...
tar xzf atomfind_data_v1.tar.gz \
    --strip-components=1 -C atomfind/data/
python atomfind/run_atomfind.py \
       --preset NL70_coherent --out ./out
python atomfind/polarisation.py --out ./out
\end{verbatim}}
executed from the directory containing \texttt{atomfind/}. The run requires a few minutes on a
