...
itself, although weighted over both oxygen sites the full edge retains $39$--$46\%$ of that
dichroism. Recovering
...
the weighted full O~K edge to $39$--$46\%$ rather than $78\%$. That range spans two systematics,
the chemical shift between the two sites, which these calculations do not fix and to which the
integral metric is much less sensitive at $30$--$32\%$, and the second equatorial orientation,
equivalent to the first for $\mathbf{q}\parallel c$ but not for $\mathbf{q}\perp c$, which would
need a third momentum-transfer direction to settle. The rotational-invariance test passes
...
$39$--$46\%$ of that dichroism. The dipole limit nevertheless forbids recovery of the sign, and the magic
...
weighted O~K edge is pinned only to $39$--$46\%$; narrowing it needs one further
momentum-transfer direction on the equatorial site and a chemical shift from a separate no-hole
calculation. Beyond that, a faithful vortex simulation would inject a per-atom $\sigma(E)$
