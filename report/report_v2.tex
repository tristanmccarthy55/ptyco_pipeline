...
off-centring \citep{Bugnet2016}, while Ti~L$_{2,3}$ was treated for anisotropy
...
\begin{table}[t]
  ...
  \begin{tabular}{@{}lccccc@{}}
    ...
    \quad bulk, $z=10$--$56$\,\AA & 95.6 & 96.4 & 95.5 & --- & --- \\
    \bottomrule
  \end{tabular}
...
Ti--O--Ti chain. The excited site here is the apical oxygen, which lies on that chain parallel
to the polar axis. The edge of the cell as a whole is the multiplicity-weighted sum of one
apical and two equatorial oxygen, and the equatorial site behaves very differently: its own
chain lies in-plane, and it is only $22\%$ dichroic. Carrying twice the multiplicity, it brings
the weighted full O~K edge to $35.8\%$ rather than $78\%$. The two equatorial atoms are equivalent
for $\mathbf{q}\parallel c$ but not for $\mathbf{q}\perp c$, so the second orientation was computed
explicitly as a third momentum-transfer direction rather than estimated. Combining two sites also
requires the core-level shift between them, which the spectra themselves do not carry; because
both core-hole calculations share a supercell and a ground state, the difference of their
total energies supplies it directly as $0.171$\,eV, leaving the weighted edge with no free
parameter. The rotational-invariance test passes
essentially exactly: mapped into the crystal frame the $c\parallel z$ and $c\parallel x$ cells
agree to $0.02$--$0.03\%$, the floor set by the cubic null test, so the anisotropy is physical
rather than an artefact of the momentum-transfer machinery.
...
denoising of the volume before fitting is a natural route below $10^{6}$\,e\,\AA$^{-2}$. The
...
Sec.~\ref{sec:pol}. It is packaged as a standalone repository containing the localisation code
alone, together with a checksummed archive of four inputs: the reconstructed phase volume as a
