...
peak detection reaches at most $68\%$ over the same band. That advantage is concentrated entirely in the axially
...
component remains spectroscopically accessible. The apical-oxygen O~K edge is $78\%$ dichroic to
the polar-axis orientation, and the multiplicity-weighted edge of the whole cell $35.8\%$, of
which a monotonic $19\%$ change in the along-beam-probed spectrum belongs to the off-centring
rather than to the tetragonal backbone. What limits the measurement is the probe convergence
rather than the collection aperture, so a hollow detector loses nothing that a smaller aperture
would recover.
\end{abstract}
...
Ferroelectric oxide superlattices host polar textures with no counterpart in the bulk. Ordered
arrays of vortex--antivortex pairs were first resolved in PbTiO$_3$/SrTiO$_3$ (PTO/STO)
superlattices \citep{Yadav2016}, and room-temperature polar skyrmions were subsequently found in
the same system \citep{Das2019}; the resulting family of topological polar phases is reviewed by
\citet{Junquera2023}. In these superlattices, competition between elastic, electrostatic and
...
Multislice electron ptychography (MEP) provides an alternative. Replacing the single
multiplicative projection with a stack of transmission slices \citep{MaidenHumphryRodenburg2012}
allows the specimen to be sectioned optically from one orientation, as first demonstrated for
electrons by \citet{Gao2017}. Because its lateral resolution is set by the illumination angle
rather than by the lens \citep{Nellist1995}, the method has reached deep sub-\aa{}ngstr\"om
resolution \citep{Jiang2018} and works even on uncorrected instruments \citep{Nguyen2024}. It
has since been applied to dopant depths in bulk crystals \citep{Chen2021}, to oxygen vacancy
sites in nickelates \citep{Dong2024}, and, by coupling a few small-angle projections, to
sub-nanometre depth resolution \citep{Dong2025}. Atomic electron tomography offers quantified
three-dimensional coordinates \citep{Yang2017} but requires a large tilt series, which is
prohibitive for a beam-sensitive oxide.
...
through the specimen by the multislice recursion \citep{CowleyMoodie1957}
\begin{equation}
...
\citep{RodenburgFaulkner2004}, the single multiplicative projection being replaced
...
$\{K(\cdot-\mathbf{r})\}$ \citep{MallatZhang1993}, iteration being stopped at a
...
\citep{Chen2021}. Columns are then typed from their own fitted-amplitude
...
\citep{Chen2021}. On a dense sublattice the per-peak form is the bound obtained
...
\citep{Vovk2005} in its Mondrian, label-conditional variant, stratified by species, detection
mode and depth band, so that coverage holds per stratum rather than only on average. The $95\%$ interval is the default in released
...
core-to-conduction matrix elements were contracted by OptaDOS \citep{Morris2014}
...
off-centring in BaTiO$_3$ \citep{Bugnet2016} and in PTO/STO superlattices at the unit-cell
scale \citep{TorresPardo2011}, while Ti~L$_{2,3}$ was treated for anisotropy
...
\begin{table}[t]
  ...
  \caption{Recovery of each sublattice, whole field unless stated, on the same reconstruction
    with the same measured kernel. The bulk band is $z=10$--$56$\,\AA. Isolated oxygen
    occupies its own column; overlapped oxygen
...
here is consequently a lower bound. It is tempting to expect the margin to widen at finite
dose; Sec.~\ref{sec:dose} shows that this cannot be tested at a single operating point, because
the baseline saturates its detection limit there. A peak-picker cannot, at any dose, provide a species label or a
...
inflation of the joint fit is negligible at the $3.9$\,\AA{} heavy-atom spacing but reaches
$2.02$ in depth at the $1.95$\,\AA{} Ti--O separation, so a per-peak bound understates
$\sigma_z$ by $\approx1.4\times$ for every apical oxygen, and most atoms in the field have such a
neighbour (Appendix~\ref{app:uq}). Inverting the full Fisher matrix restores the inflation per
...
\begin{table}[tb]
  ...
  \caption{Dose series on an independent reconstruction ($105$ layers, $0.10$\,\AA{} sampling),
    run with the same configuration as the coherent reference. Recalls are over the bulk band.
    The $10^{6}$ row illustrates the uninformativeness of precision, which reads $0.96$ while
    oxygen recall is $5\%$. At $10^{4}$ the method returns no atoms at all rather than reporting
    noise.}
  \label{tab:dose}
\end{table}
...
$10^{6}$. The residual gap to the coherent reference (Ti $74\%$ against $96\%$ on the same bulk band) is attributable
...
The baselines of Sec.~\ref{sec:bench} were run over the same series, expecting the oxygen margin
to widen once noise was present. It does not, for an instructive reason: peak detection is
subject to a cap on the number of maxima returned, and it reaches that cap at every dose,
returning the same $2500$ detections at $10^{10}$ as at $10^{4}$. Recall is then set by the cap
rather than by the data, and the ordering becomes impossible, axially overlapped oxygen scoring
$71$--$77\%$ against $45$--$49\%$ for the easier isolated oxygen. Counting true detections shows
why: at $10^{8}$ the model fit makes $1404$ from $1593$ returned and peak-picking $1407$ from
$2500$, buying the same true detections with $1093$ false ones. Discriminating at finite dose
would need the baselines re-thresholded to a matched count, or a precision-recall curve rather
than one operating point, so no claim is made from it here. What the series does establish is
the value of abstention: at $10^{4}$ the model fit returns nothing, whereas maximum-entropy
deconvolution returns $3803$ detections at a precision of $0.34$.

The series carries a further lesson that outlives this dataset. Precision is not a safety
indicator: at $10^{6}$\,e\,\AA$^{-2}$ it reads $0.96$ while titanium recall is $26\%$, oxygen
$5\%$ and $38\%$ of labels are wrong, because it measures only the surviving bright lead atoms,
which are correctly placed. Species confusion and $\sigma$-coverage are the diagnostics that respond, and
...
and reduced confusion from $19.0\%$ to $12.4\%$ at $10^{10}$\,e\,\AA$^{-2}$. Sub-\aa{}ngstr\"om
...
strongly dichroic, at $\max|\Delta|/\max S = 78.2\%$ and $\int|\Delta|/\int S = 62.8\%$
(Fig.~\ref{fig:eels}). The
...
Ti--O--Ti chain. The excited site here is the apical oxygen, which lies on that chain
parallel to the polar axis. The edge of the cell as a whole is the multiplicity-weighted sum of
one apical and two equatorial oxygen, and the equatorial site behaves very differently: its own
chain lies in-plane, and it is only $22\%$ dichroic. Carrying twice the multiplicity, it brings
the weighted full O~K edge to $35.8\%$ rather than $78\%$, with no free parameter left in the
combination (Appendix~\ref{app:geom}). The rotational-invariance test passes essentially
exactly: mapped into the crystal frame the $c\parallel z$ and $c\parallel x$ cells agree to
$0.02$--$0.03\%$, the floor set by the cubic null test, so the anisotropy is physical rather
than an artefact of the momentum-transfer machinery.
...
$\beta=1$\,mrad, $0.28$ at $2$\,mrad, $-0.04$ at $5$\,mrad and $-0.33$ at $100$\,mrad. Read
naively this appears to condemn a large collection angle, and so to condemn the combined
experiment, in which one pixelated detector carries a central hole and the spectrometer therefore
collects out to $\beta\approx100$\,mrad.

That calculation is nevertheless the textbook one, which assumes parallel illumination and is
the only geometry for which a magic angle is defined. A focused probe is not that geometry: the
momentum transfer is a vector difference, so the convergence semi-angle spreads $\mathbf{q}$
however small the spectrometer aperture is. Averaging over both cones
(Appendix~\ref{app:geom}) shows that closing the collection aperture is not by itself a remedy,
but also that the anisotropy does not vanish beyond the magic angle: it inverts and saturates
near a third of its intrinsic value, and at $\alpha=100$\,mrad the surviving fraction is flat at
$-0.32$ to $-0.34$ for every collection angle from $1$ to $100$\,mrad. Folding the measured
weighted edge through this geometry, a domain with its polar axis along the beam differs from one
with the axis in-plane by $12.1\%$ in the collected O~K edge, needing
$\approx1.2\times10^{3}$ counts per channel for SNR $3$. The spectrometer aperture is therefore
not a design variable, and a hollow detector that gives the spectrometer the whole bright-field
disc costs the spectroscopy nothing a smaller aperture would recover.
...
(interquartile range $73$--$87^\circ$), an along-beam fraction of only $\approx0.14$.

That last point does not, however, make the computed case an upper bound that a real measurement
would fall far below. Repeating the calculation on a cell built at the labyrinth's median
$82^\circ$ tilt gives a dichroism of $76.3\%$, against $78.2\%$ for the fully aligned case. What
changes is not the magnitude but the character, and exactly so: with the polar axis along the
beam, $\mathbf{q}\parallel$~beam gives the $\sigma^*$ feature at $18.7$\,eV and
$\mathbf{q}\perp$~beam the $\pi^*$ at $8.6$\,eV, whereas with the axis in-plane the same two
features appear at the same two energies with their assignments exchanged. The edge reports the
\emph{orientation} of the polar axis relative to the beam, the sense of the asymmetry
identifying which way it lies, rather than the magnitude of the along-beam component. A
measurement on the labyrinth would therefore return a large dichroism reporting an in-plane axis,
not an absence of signal. It remains true that only a small part of the effect belongs to the
off-centring rather than to the tetragonal backbone, and that the dipole limit forbids the sign.
...
the established deconvolve-then-detect workflow does not, at $95.5\%$ bulk recall against at
most $68\%$ over the same band. The whole of that margin lies in the axially overlapped oxygen ($82\%$ against
...
spectroscopically accessible in principle. The apical-oxygen O~K edge is $78\%$ dichroic to the
polar-axis orientation, and the multiplicity-weighted edge of the whole cell $35.8\%$, of which a
monotonic $19\%$ change in the along-beam-probed spectrum is the signature of the displacement
rather than of the tetragonal backbone. Two limits remain. The dipole term is even in
$\mathbf{q}$ and so forbids recovery of the sign, and the anisotropy is averaged by the probe
convergence rather than by the collection aperture, which cannot be undone by closing the
spectrometer entrance and places the full effect out of reach of a strongly convergent probe.
...
denoising of the volume before fitting \citep{Wu2026} is a natural route below
$10^{6}$\,e\,\AA$^{-2}$. The
weighted O~K edge is now free of adjustable parameters, so the remaining uncertainty in it is
that of the underlying functional rather than of the analysis. Beyond that, a faithful vortex simulation would inject a per-atom $\sigma(E)$
keyed to each oxygen's local polar-axis orientation. The clearest instrumental conclusion is that a
hollow detector is the right geometry, for a reason that is not the obvious one. A convergent
probe spreads the momentum transfer by itself, so beyond the magic angle the anisotropy inverts
and saturates near a third of its intrinsic value rather than vanishing, and at the convergence
imaging requires it is independent of the collection angle to within a few percent. Nothing is
therefore lost by giving the spectrometer the central disc and the imaging the outer annulus. The
limit on such an experiment in this material is not the detector but the specimen: the polar axes
of the labyrinth are clustered about the in-plane direction, so the orientation contrast between
neighbouring domains is of order $1\%$ rather than the $12\%$ available between the limiting
orientations, and a zone axis presenting a wider spread of axes would serve far better. The open question is whether the two can be fused into a
single three-dimensional vector polarisation measurement remains open; energy-filtered
ptychography of the oxygen sublattice \citep{Dong2024} suggests one route.
...
\begin{table}[t]
  ...
  \begin{tabular}{@{}lll@{}}
    ...
    Probe  & energy / $\alpha$ & $300$\,keV / $100$\,mrad \\
           ...
           & higher aberrations & none ($C_{30}=C_{12}=0$) \\
           & coherence & fully coherent; no source size or \\
           &            & energy spread imposed \\
    \addlinespace
    ...
    DFT    & code / functional & CASTEP \citep{Clark2005} / PBE \\
           ...
           & $k$-grid & $4\times4\times4$ Monkhorst--Pack, SCF \\
           &          & and spectral \\
    ...
    ELNES  & code / task & OptaDOS \citep{Morris2014}, core-loss \\
           ...
           & broadening & Gaussian $1.0$\,eV (instrument), \\
           &            & Lorentzian $0.5$\,eV (lifetime) \\
    \bottomrule
  \end{tabular}
  \caption{Simulation, reconstruction and first-principles parameters. The probe carries only
    defocus: the simulated instrument is an ideal aberration-corrected one, and the illumination
    is fully coherent, so the only incoherence in the data is thermal diffuse scattering from the
    frozen-phonon average and Poisson counting noise. The mixed-state probe used in the dosed
    reconstructions therefore absorbs thermal scattering rather than a modelled source
    incoherence.}
  \label{tab:params}
\end{table}
...
single-column precision \citep{DeBacker2016} and single-dopant depth \citep{Chen2021}; on a
dense sublattice it is the bound obtained with all neighbours known exactly. Table~\ref{tab:vif} gives the
...
five libraries, and run end to end. It reproduced every quantity below, the largest relative
discrepancy anywhere in the emitted record being $1.3\times10^{-11}$, on a mean null-site
...
The O~K edge of the cell is the multiplicity-weighted sum of one apical and two equatorial
oxygen. The two equatorial atoms are related by the four-fold axis and are equivalent for
$\mathbf{q}\parallel c$, but not for $\mathbf{q}\perp c$, so the second in-plane orientation was
computed explicitly as a third momentum-transfer direction rather than proxied by a similar one.
That distinction matters: proxying it by the $\mathbf{q}\parallel c$ spectrum gives $39.1\%$ and
counting the first equatorial atom twice gives $46.4\%$, whereas the measured value is $37.1\%$,
below both. Combining sites also requires the core-level shift between them, which the spectra
do not carry, since OptaDOS writes each on a Fermi-referenced axis. Both core-hole calculations
share a supercell and hence a ground state, so the difference of their total energies is the
final-state shift directly, $0.171$\,eV, and displacing the equatorial edge by it moves the
weighted result from $37.1\%$ to $35.8\%$. This is well inside the $\pm1$\,eV sensitivity sweep
that previously stood as the systematic, so the weighted edge carries no free parameter.

\subsection{Aperture averaging}
% ===========================================================================

That calculation is nevertheless the textbook one, which assumes parallel illumination, and it is
the only geometry for which a magic angle is defined. A focused probe is not that geometry. The
momentum transfer is the vector difference of the incident and scattered transverse angles, so a
convergence semi-angle $\alpha$ spreads $\mathbf{q}$ on its own, irrespective of how small the
spectrometer aperture is. Averaging the dipole weights over both cones changes the picture. At
$\beta=1$\,mrad the surviving fraction falls from $0.62$ under parallel illumination to $0.29$ at
$\alpha=2$\,mrad and to $-0.04$ at $\alpha=5$\,mrad. Closing the collection aperture is therefore
not by itself a remedy, and recovering the \emph{full} anisotropy would require illumination as
parallel as $\theta_E=1.09$\,mrad, which is incompatible with a focused probe.

The consequence for a combined instrument is more favourable than that reads, because the
anisotropy does not vanish beyond the magic angle. It inverts and then saturates, approaching
about a third of its intrinsic value. At the $\alpha=100$\,mrad convergence that atomic-resolution
ptychography requires, the surviving fraction is $-0.32$ to $-0.34$ for every collection angle
between $1$ and $100$\,mrad. It is, in other words, flat: the spectrometer aperture is no longer a
design variable, because the convergence already sets the averaging. Folding the measured
weighted edge through this geometry, a domain with its polar axis along the beam differs from one
with the axis in-plane by $12.1\%$ in the collected O~K edge, which needs $\approx1.2\times10^{3}$
counts per channel for SNR $3$ rather than the $4.5\times10^{4}$ implied by a $2\%$ anisotropy.

This is the quantitative case for a hollow detector. Placing the spectrometer entrance in the
centre of the pixelated detector and reserving the outer annulus for imaging costs the
spectroscopy nothing that a smaller aperture would recover, while collecting the entire
bright-field disc gives it the maximum available signal. The hole radius may be chosen to suit
the imaging.

*The treatment here is kinematic and dipole-limited, and averages incoherently over incident
directions. It carries neither channelling nor thickness dependence, both of which belong to a
multislice core-loss simulation of the instrument and are left as further work.
