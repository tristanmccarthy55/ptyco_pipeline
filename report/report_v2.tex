...
spectrometer aperture is. Averaging the dipole weights over both cones changes the conclusion
materially. At $\beta=1$\,mrad the surviving fraction falls from $0.62$ under parallel
illumination to $0.29$ at $\alpha=2$\,mrad and to $-0.04$ at $\alpha=5$\,mrad, and beyond
$\alpha\approx5$\,mrad no choice of $\beta$ recovers it. Closing the collection aperture is
therefore not sufficient: the convergence must itself be brought down to the order of
$\theta_E=1.09$\,mrad. At a $2\%$ fractional anisotropy the measurement needs
$\approx4.5\times10^{4}$ counts per channel for SNR $3$, which sets the dose target.
...
design constraint, and a stronger one than a choice of aperture. Because a convergent probe
spreads the momentum transfer by itself, the anisotropy survives only for illumination whose
convergence approaches $\theta_E$, of order a milliradian. Depth-resolved imaging requires the
opposite, a probe convergent enough to be focused to atomic dimensions, so the two measurements
are incompatible in illumination mode and not merely in the aperture they collect through. The open question is whether the two can be fused into a
